...
export default function HiddenCommandTab({ asrKeys, loading, error, result, onRun, onTransfer }) {
  ...
  const go = async () => {
    ...
    fd.append('carrier_file', file); fd.append('model', model); fd.append('command_text', cmd)
  ...
  return (<>
    <Card><SectionLabel>Input</SectionLabel>
      <InputGrid>
        <AudioInput label="Carrier audio" file={file} setFile={setFile} />
        <Select label="ASR Model" value={model} onChange={setModel} options={asrKeys} />
        <TextInput label="Hidden command" value={cmd} onChange={setCmd} placeholder="e.g. open the door" />
      </InputGrid>
      <div className="mt-3"><PreviewTranscription file={file} model={model} /></div>
    </Card>
    <Card><SectionLabel>Parameters</SectionLabel>
      <ParamGrid>
        <Slider label="Epsilon" value={eps} onChange={setEps} min={0.01} max={0.5} step={0.01} defaultValue={0.1} />
    ...
    <div className="flex items-center gap-4"><RunButton onClick={go} loading={loading} label="Embed Hidden Command" /><ErrorMsg msg={error} /></div>
    {result && <AsrResult result={result} onTransfer={() => onTransfer?.({ originalWavB64: origB64, targetText: cmd })} originalWavB64={origB64} />}
  </>)
}
